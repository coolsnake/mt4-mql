...
input bool            RSI                  = false;                     // Relative String Index (RSI)
input bool            RVI                  = false;                     // Relative Vigor Index (RVI)
input bool            CCI                  = false;                     // Commodity Channel Index (CCI)
...
int OnInit() {
   ...
   if (time_frame < _Period) {
      Print("Invalid input signal timeframe ("+ IntegerToString(time_frame) +") is less than trading timeframe ("+ IntegerToString(_Period) +")");
      return(INIT_PARAMETERS_INCORRECT);
   }
   if (BE_Pips > Trail_Stop_Pips && Trail_Stop_Pips > 0) {
      Print("Breakeven pips ("+ IntegerToString(BE_Pips) +") is greater than trailing stop ("+ IntegerToString(Trail_Stop_Pips) +")");
      return(INIT_PARAMETERS_INCORRECT);
   }
   if (!loop_back_bars && !MA_Period && !RSI && !RVI && !CCI) {
...
void move_to_BE() {
   ...
   if (enable_atr) trail_stop_pips = (int)(atr * ATR_BE/Point);
